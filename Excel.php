...
class Excel extends \yii\base\Widget
{
	...
	/**
     * @var boolean to maintain cell formats when imporing data, or just import the data itselfi in raw format
	 */
    public $readCellFormat = true;
		...
		if ($sheetCount > 1) {
			foreach ($objectPhpExcel->getSheetNames() as $sheetIndex => $sheetName) {
				if (isset($this->getOnlySheet) && $this->getOnlySheet != null) {
					...
					$sheetDatas[$indexed] = $objectPhpExcel->getActiveSheet()->toArray(null, true, $this->readCellFormat, true);
    ...
    {
        if (isset($config['readCellFormat'])) $this->getFormat = $config['readCellFormat'];
